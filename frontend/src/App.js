...
function App() {
  ...
  const [user, setUser] = useState(null);
  const [currentTime, setCurrentTime] = useState(new Date());
  const [activeView, setActiveView] = useState('main'); // 'main', 'desk', 'gate', 'display'
  ...
  const backToMain = () => setActiveView('main');
    ...
    <div className="App">
      ...
      {activeView === 'main' && (
        <>
          ...
          {user.role === 'display' && <DisplayPanel onBack={() => setActiveView('main')} />}
        </>
      )}

      {activeView === 'desk' && <DeskOperator deskNumber={deskNumber} onBack={backToMain} />}
      {activeView === 'gate' && <GateOperator gateNumber={gateNumber} onBack={backToMain} />}
      {activeView === 'display' && <DisplayPanel onBack={() => setActiveView('main')} />}
    </div>
  );
}
